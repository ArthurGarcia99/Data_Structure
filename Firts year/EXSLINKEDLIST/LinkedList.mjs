...
export default class LinkedList {
    ...
    #removedHistory // lista com valores dos nos removidos
    ...
    remove(pos) {
        ...
        //adiociona o valor do nodo removido na removedHistory
    ...
    // Metodo para retornar elementos removidos
    history(){
        if(this.#removedHistory.length === 0){
            return "Histórico de remoções = Nenhuma remoção ainda."
        }else{
            return "Histórico de remoções = " + this.#removedHistory
        }
    }

    // Metodo para inverter a lista encadeada
    reverse(){
        if(this.isEmpty || this.#count === 1) return -1

        let ultimo = this.#tail.data
        this.#tail.data = this.#head.data
        this.#head.data = ultimo
    }

    // Metodo para conferir quantas vezes um elemento aparece nela
    countOccurences(val){
        let node = this.#head
    ...
    // Metodo para remover todos as occorencias de um valor
    removeAll(val){
    ...
    // Metodo transformar a lista em um array
    toArray(){
        let lista = []
        let node = this.#head

        for(let i = 0; i < this.#count; i++){
            lista.push(node.data)
